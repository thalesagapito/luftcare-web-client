...
// import DoctorOverviewQueryGQL from '~/graphql/queries/User/doctorOverview';
...
export const DOCTOR_PATH = '/dashboard/medicos/:id';
...
export default Vue.extend<Data, Methods, Computed, Props>({
  ...
  head() {
    const { doctorName } = this;
    return {
      titleTemplate(base) {
        return `${base} - Acompanhar médico ${doctorName}`;
      },
    };
  },
  validate({ params }) {
    const { id } = params;
    if (!id) throw new Error('Identificador de médico ausente na url');
    if (!isUUID(id)) throw new Error('Identificador de médico inválido');
    return true;
  },
});
</script>
